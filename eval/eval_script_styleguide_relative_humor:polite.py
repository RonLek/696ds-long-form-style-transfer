...
for prompt_index, prompt in enumerate(['self_discover_paired']):
    output_data =[['normalized_humor', 'normalized_politeness']]
    with open('output_relative_self_discover.csv', 'r') as f_outputs: #combined dataset
        ...
        for pair_index in range(100):  # 100 pairs, 20 per domain
            line = next(reader_obj)
            try:
                humor_score = attribute_scorer(humor_tokenizer, humor_model, 1, dataset_benchmark.iloc[pair_index]['content2'], line[6 + prompt_index],pair_index)
                politeness_score = attribute_scorer(politeness_tokenizer, politeness_model, 1, dataset_benchmark.iloc[pair_index]['content2'], line[6 +
