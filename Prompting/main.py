...
class StyleTransfer:
    ...
    def run_prompting(self, source_doc, publication_name, num_references=None):
        reference_doc = self.publications_df.loc[self.publications_df['Publication_name'] == publication_name, 'reference_doc'].sample(n=num_references)
    ...
    def process_dataset(self, num_docs=0, num_references=None):
        ...
        with tqdm(total=num_docs, desc="Processing documents", unit="doc") as pbar:
            for index, row in self.paired_df.iloc[:num_docs].iterrows():
                ...
                zero_shot_result, few_shot_result, self_discover_result = self.run_prompting(source_doc, publication_name, num_references)
