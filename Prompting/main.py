...
class StyleTransfer:
    ...
    def run_prompting(self, source_doc, publication_name, num_references=None):
        ...
        try:    
            ...
            print(f"Encountered a type error in zero-shot prompting: {e}")
            zero_shot_result = ""
    ...
    "26. Develop a process for evaluating the effectiveness of the style guideline and gathering feedback from writers, editors, and readers to inform future iterations and improvements."
        ...
        try:
            # Self-discover
            self_discover_result = perform_self_discover(source_doc, reference_doc, publication_name, reasoning_modules)
        except TypeError as e:
            print(f"Encountered a type error in self-discover prompting: {e}")
    ...
    def process_dataset(self, num_docs=0, num_references=None):
        ...
        with tqdm(total=num_docs, desc="Processing documents", unit="doc") as pbar:
            for index, row in self.paired_df.iloc[:num_docs].iterrows():
                ...
                publication_name = row.iloc[4]  # Assumes 'Pub2' is the fifth column (index 4)
                
                try:
                    zero_shot_result = self.run_prompting(source_doc, publication_name, num_references)[0]
                    self.save_output_csv(index, 'Zeroshot_output', zero_shot_result)
                ...
                try:
                    ...
                    self.save_output_csv(index, 'SelfDiscover_output', self_discover_result)
                except Exception as e:
                    print(f"Encountered an error in self-discover prompting: {e}")
    ...
    def save_output_csv(self, index=None, column_name=None, result=None):
        ...
        if index is not None and column_name is not None and result is not None:
            if column_name not in self.paired_df.columns:
                self.paired_df[column_name] = ''
            self.paired_df.at[index, column_name] = result
